...
public sealed class Postgresql16DatabaseSchemaProvider : PostgresqlDatabaseSchemaProvider
{
    ...
    public Postgresql16DatabaseSchemaProvider()
    {
    }

    /// <summary>
    /// A per-build instance carrying the declared target floor, so a project that names a point
    /// release (PostgreSQL 16.x.y) gates against what it actually declared rather than this major's
    ...
    /// </summary>
    public Postgresql16DatabaseSchemaProvider(TargetVersion? targetVersion)
        : base(targetVersion)
    {
    }
