...
public sealed class Postgresql15DatabaseSchemaProvider : PostgresqlDatabaseSchemaProvider
{
    ...
    public Postgresql15DatabaseSchemaProvider()
    {
    }

    /// <summary>
    /// A per-build instance carrying the declared target floor, so a project that names a point
    /// release (PostgreSQL 15.x.y) gates against what it actually declared rather than this major's
    ...
    /// </summary>
    public Postgresql15DatabaseSchemaProvider(TargetVersion? targetVersion)
        : base(targetVersion)
    {
    }
