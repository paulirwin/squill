...
/// column). Rendering is centralized here so every model builder produces identical
/// names — a prerequisite for comparing a model built from parsed SQL against one
/// extracted from a live database.
///
/// The canonical (in-memory) form is unquoted and dot-joined (e.g. film.title); quoting
/// is a SQL-serialization concern, produced on demand via <see cref="Sql"/> and
/// <see cref="QuotedUnqualified"/>. MariaDB quotes identifiers with backticks.
/// </summary>
public sealed class SqlName : IEquatable<SqlName>
{
    ...
    /// segment (e.g. an index in the same table's namespace).
    /// </summary>
    ...
    /// <summary>The last segment, backtick-quoted but not qualified (e.g. `title`).</summary>
    public string QuotedUnqualified => $"`{_segments[^1]}`";

    /// <summary>The fully qualified, backtick-quoted SQL rendering (e.g. `film`.`title`).</summary>
    public string Sql => string.Join('.', _segments.Select(s => $"`{s}`"));

    /// <summary>The canonical, unquoted, dot-joined rendering (e.g. film.title).</summary>
    public override string ToString() => string.Join('.', _segments);

    /// <summary>
    /// Parses a canonical (unquoted, dot-joined) name — as stored in a model element's
    /// Name — and returns its last segment. Used where only the bare identifier is needed,
    /// e.g. a column name inside a CREATE TABLE body.
    /// </summary>
