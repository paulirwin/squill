...
public sealed class MariaDb12DatabaseSchemaProvider : MariaDbDatabaseSchemaProviderBase
{
    ...
    public MariaDb12DatabaseSchemaProvider()
    {
    }

    /// <summary>
    /// A per-build instance carrying the declared target floor, so a project that names a point
    /// release (MariaDB 12.x.y) gates against what it actually declared rather than this major's
    ...
    /// </summary>
    public MariaDb12DatabaseSchemaProvider(TargetVersion? targetVersion)
        : base(targetVersion)
    {
    }
    ...
    /// <inheritdoc />
    // Measured on 12: unchanged from 11.
    protected override (string Utf8Mb3, string Utf8Mb4) DefaultUnicodeCollations
        => ("utf8mb3_uca1400_ai_ci", "utf8mb4_uca1400_ai_ci");
