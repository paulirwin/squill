...
public class PostgresSimpleTableTest : PostgresIntegrationTestBase
{
    ...
    public async Task SimpleCreateTableTest()
    {
        ...
        var testDb = await provider.CreateDatabaseAsync($"squill_test_{Guid.NewGuid():n}", TestContext.Current.CancellationToken);
        var dbModelBuilder = provider.CreateDatabaseModelBuilder(testDb);

        try
        {
            var testModel = await dbModelBuilder.ExtractModelAsync(TestContext.Current.CancellationToken);
            ...
            var newModel = await dbModelBuilder.ExtractModelAsync(TestContext.Current.CancellationToken);
