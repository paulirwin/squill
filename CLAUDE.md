...
Early scaffolding for serializing a model to the DACPAC zip format (`Origin.xml`, `DacMetadata.xml`, `model.xml`, `[Content Types].xml`), with the goal of byte-compatible output with SSDT-built DACPACs. Optional `predeploy.sql` / `postdeploy.sql` parts carry pre/post-deployment scripts (issue #67): they are stored verbatim (never parsed into the model), checksummed in `Origin.xml` like `model.xml`, surfaced as `ModelMetadata.PreDeployScript`/`PostDeployScript`, and composed around the schema diff by `DeploymentScripts.Compose`. Deploy scripts run on every deploy, including one with zero schema deltas. Note the XML writer prototypes (`ModelWriter`, `OriginWriter`, etc.) currently live in `Squill.Dacpac.Tests` — this is work in progress.
